...
export function get(params, query, router) {
    ...
    return Promise.all([
        ...
        .then(() => {
            return data.getCategoryData(params.id, pageSize, pageNumber);
        })
        .then(([
                [categoryData, posts],
                fromTheBlogData,
            ...
            categoryData.root = 'Home';
            const pages = [];
            const pageCount = Math.ceil( categoryData.postCount / pageSize );
            for ( let i = 1; i <= pageCount; i += 1 ) {
                pages.push( {
                    href: `/category/${categoryData._id}?pagenumber=${i}`,
                    index: i,
                });
            }
